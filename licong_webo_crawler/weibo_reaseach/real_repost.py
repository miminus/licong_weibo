...
for page in range(comm_page):
	url='http://weibo.com/aj/v6/mblog/info/big?ajwvr=6&id='+post_id+'&page='+str(page+1)
	#print url
	driver.get(url)
	print driver.title
	# print driver.current_url  #获取当前网页url
	# print driver.name
	f=open('%s/page_repost.txt' %setting.CURRENT_PATH,'wb')
	time.sleep(5)
	...
	f.write(con)
	f.close()
	os.system('python %s/parse_repost.py' %setting.CURRENT_PATH)
	#########################################################################################
	time.sleep(2)
driver.quit()