...
query='select key_wd from key_word where id=1'
cur.execute(query)
db.commit()
word=cur.fetchone()
...
word = urllib.quote(word).replace('%','%25')
# print word
# post_url_quote=geturl()   #获取url编码

baseurl='http://s.weibo.com/wb/'+word+'&xsort=hot&scope=ori&vip=1&timescope=custom:2014-04-1:2014-04-12&nodup=1&page='
page=1
